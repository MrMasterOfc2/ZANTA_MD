...
const downloadMediaMessage = async(m, filename) => {
	// ... (Download Logic එලෙසම) ...
	if (m.type === 'viewOnceMessage' || m.type === 'viewOnceMessageV2') {
		m.type = m.msg.type
	}
    
    // 🚩 FIX A: JID Normalization භාවිතයෙන් remoteJid එක නිවැරදි කරන්න
    if (m.key?.remoteJid) {
...
const sms = (zanta, m) => {
	if (m.key) {
		m.id = m.key.id
        // 🚩 FIX: JID Normalization (m.chat එක සාමාන්‍යකරණය කිරීම)
		m.chat = jidNormalizedUser(m.key.remoteJid)
		m.fromMe = m.key.fromMe
		m.isGroup = m.key.remoteJid.endsWith('@g.us')
        // 🚩 FIX: JID Normalization (m.sender එක සාමාන්‍යකරණය කිරීම)
		m.sender = m.fromMe ? zanta.user.id.split(':')[0]+'@s.whatsapp.net' : m.isGroup ? jidNormalizedUser(m.key.participant) : jidNormalizedUser(m.key.remoteJid)
	}
	if (m.message) {
    ...
    // 🚩 FIX: m.reply function එකේ JID එකද Normalize කිරීම
	m.reply = (teks, id = m.chat, option = { mentions: [m.sender] }) => zanta.sendMessage(jidNormalizedUser(id), { text: teks, contextInfo: { mentionedJid: option.mentions } }, { quoted: m })
	m.replyS = (stik, id = m.chat, option = { mentions: [m.sender] }) => zanta.sendMessage(jidNormalizedUser(id), { sticker: stik, contextInfo: { mentionedJid: option.mentions } }, { quoted: m })
	m.replyImg = (img, teks, id = m.chat, option = { mentions: [m.sender] }) => zanta.sendMessage(jidNormalizedUser(id), { image: img, caption: teks, contextInfo: { mentionedJid: option.mentions } }, { quoted: m })
	m.replyVid = (vid, teks, id = m.chat, option = { mentions: [m.sender], gif: false }) => zanta.sendMessage(jidNormalizedUser(id), { video: vid, caption: teks, gifPlayback: option.gif, contextInfo: { mentionedJid: option.mentions } }, { quoted: m })
	m.replyAud = (aud, id = m.chat, option = { mentions: [m.sender], ptt: false }) => zanta.sendMessage(jidNormalizedUser(id), { audio: aud, ptt: option.ptt, mimetype: 'audio/mpeg', contextInfo: { mentionedJid: option.mentions } }, { quoted: m })
	m.replyDoc = (doc, id = m.chat, option = { mentions: [m.sender], filename: 'undefined.pdf', mimetype: 'application/pdf' }) => zanta.sendMessage(jidNormalizedUser(id), { document: doc, mimetype: option.mimetype, fileName: option.filename, contextInfo: { mentionedJid: option.mentions } }, { quoted: m })
